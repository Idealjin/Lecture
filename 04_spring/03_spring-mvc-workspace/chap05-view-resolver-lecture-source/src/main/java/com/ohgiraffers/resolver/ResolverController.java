...
public class ResolverController {
	...
	public String stringRedirect(Model model) throws UnsupportedEncodingException {
		...
		//접두사로 redirect: 를 하면 forword가 아닌 redirect시킨다.
	...
	public String stringRedirectFlashAttribute(RedirectAttributes rttr) {
		...
		/*
		 * 리다이렉트시 url에 노출되지 않는 정보를 flash영역에 한 번만 담아서 redirect 할 수 있다.
		 * 자동 모델에 추가되기 때문에 requestScope에서 값을 꺼내서 사용하면된다.
		 * 세션에 임시로 값을 담고 소멸하는 방식이기 때문에 session에 동일한 키값이 존재하지 않아야 한다.
		 * */
		rttr.addFlashAttribute("flashMessage", "라다이렉트 attr 사용하여 redirect..");
	...
	public ModelAndView modelAndViewReturning(ModelAndView mv) {
		...
		/* 모델과 뷰를 합친 개념이다.
		 * 핸들러 어댑터가 핸들러 메소드를 호출하고 반환받은 문자열을 ModelAndView로 만들어서 dispatcherServlet에 반환한다.
