...
public class OhgiraffersStudent implements Student {
	...
	public AchievementResult study(Passion passion) throws Exception {
		...
		if(passion.getScore() != 10) {
			throw new Exception("OhgiraffersStudent는 열정이 MAX이어야한다. 열정이 부족합니다. 다시 열정을 가지고 오세요!!");
		}
		
		int understandingScore = 0;             // 이해도
		int satisfactionScore = 0;              // 만족도
		double employeementRage = 0.0;          // 취업율
		...
		for(int i = 1; i <= 8; i++) {
			...
			employeementRage += difficulty * passionScore;
			
			if(i == 3) {
				System.out.println("잠시 밥을 먹으며 오전에 수업 들은 내용으로 토론을 합니다. 밥을 코로먹는건지 입으로 먹는건지 잘 모르겠습니다.");
				employeementRage += difficulty * passionScore;
			}
		}
		
		return new AchievementResult(understandingScore, satisfactionScore, employeementRage);
	}
