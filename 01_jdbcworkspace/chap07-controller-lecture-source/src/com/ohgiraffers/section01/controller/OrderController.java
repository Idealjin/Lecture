...
public class OrderController {
	...
	public void registOrder(Map<String, Object> requestMap) {
		/* 컨트롤러 역할은 뷰에서 사용자가 입력한 정보를 파라미터 형태로 전달받으면
		 * 전달받은 값들을 검증하거나 추가적인 정보가 필요한 경우 가공을 한 뒤 
		 * 서비스쪽으로 전달하기 위한 인스턴스에 다고 서비스의 비지니스로직을 담당하는 메소드를 호출한다.
		...
		// 4. 서비스(비즈니스 로직)을 호출하고 결과를 리턴 받는다.
		int result = orderService.registOrder(order);
		
		
		// 5. 서비스 처리 결과를 이용해 성공 실패 여부를 판단해서 사용자에게 보여줄 뷰를 결정한다.
